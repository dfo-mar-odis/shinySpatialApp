...
makeMarfis <- function(agg.poly.shp = theSubUnitShape, agg.poly.field = theSubUnitField){
  ...
  #if no coordinate can be calculated, use the supplied one instead
  marf$NAFO <- marf$NAFO_AREA
  marf[!is.na(marf$UnitArea), "NAFO"] <- marf[!is.na(marf$UnitArea), "UnitArea"]
  
  marf[, c("UnitArea", "NAFO_AREA", "LATITUDE", "LONGITUDE", "PRO_SPC_INFO_ID")] <- list(NULL)
  marf <- marf %>%
...
makeISDB <- function(agg.poly.shp = theSubUnitShape, agg.poly.field= theSubUnitField){
  ...
  isdbCoords <- unique(isdb[!is.na(isdb$LATITUDE) & !is.na(isdb$LONGITUDE),
                            c("FISHSET_ID", "LATITUDE", "LONGITUDE")])
  isdbCoords<-  Mar.utils::identify_area(df = isdbCoords, lat.field = "LATITUDE",
                                         lon.field = "LONGITUDE", 
                                         agg.poly.shp = theSubUnitShape,
                                         agg.poly.field = theSubUnitField)
  isdbCoords$UnitArea <- toupper(isdbCoords$UnitArea)
  isdb <- merge(isdb, isdbCoords[, c("FISHSET_ID", "UnitArea")], all.x = T)
  rm(isdbCoords)
  #drop the records that are outside of the range of the shp or are invalid
  isdb <- isdb[!is.na(isdb$UnitArea) & !(isdb$UnitArea %in% c("<OUTSIDE KNOWN AREAS>",
                                                              "<IMPOSSIBLE COORD>")), ]
  
  #if no coordinate can be calculated, use the supplied one instead
  isdb$NAFO <- isdb$NAFAREA_ID
  isdb[!is.na(isdb$UnitArea),"NAFO"]<-isdb[!is.na(isdb$UnitArea),"UnitArea"]
  isdb[, c("UnitArea", "NAFAREA_ID", "LATITUDE", "LONGITUDE", "FISHSET_ID")] <- list(NULL)
  isdb <- isdb %>%
