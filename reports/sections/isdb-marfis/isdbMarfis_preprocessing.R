...
if (globalControlEnv$updateGeoms) {
  ...
  nafoGeoms_sf <- isdb_rr$data_sf
}  

isdb_rr <- list("title" = "Industry Survey Database (ISDB)",
                "data_sf" = nafoGeoms_sf,
                "attribute" = "NONE",
                "metadata" = read_google_metadata("isdb_rr")
)
save(isdb_rr, isdbSpecies_df, file = file.path(get_file_save_path(globalControlEnv$saveToRemote), "Protected/isdb_rr.RData"))
...
if (globalControlEnv$updateGeoms) {
  ...
  nafoGeoms <- marfis_rr$data_sf
}

marfis_rr <- list("title" = "Maritime Fishery Information System (MARFIS)",
                  "data_sf" = nafoGeoms_sf,
                  "attribute" = "NONE",
                  "metadata" = read_google_metadata("marfis_rr")
)
save(marfis_rr, marfisSpeceis_df, file = file.path(get_file_save_path(globalControlEnv$saveToRemote), "Protected/marfis_rr.RData"))
