...
tableList <- create_table_ISDB(outputListI$studyData, isdbSpeceis_df, listed_species)
isdbAllTableList <- tableList$allSpeciesNafoList
...
```{r ISDB-region-plot, fig.height=7, fig.width=11, fig.cap=paste(write_caption_blurb(ebsa_rr, "EN"), 'Map showing NAFO areas containing ISDB records within the search area (red outline).')}
if(!is.null(isdb_rr$data_sf)){
  plot_rr_sf(regionMap,isdb_rr$data_sf, isdb_rr$attribute, alpha=0.5)
}
```

```{r ISDB-area-plot, fig.height=7, fig.width=11, fig.cap=paste(write_caption_blurb(ebsa_rr, "EN"), 'Map showing NAFO areas containing ISDB records within the search area (red outline).')}
...
```
<br><br>
```{r isdb-all-table results="asis"}
if (!is.null(isdbAllTableList)) {
  for(tableIndex in seq_along(isdbAllTableList)) {
    print(knitr::kable(isdbAllTableList[[tableIndex]], align="l", caption = names(isdbAllTableList)[tableIndex], booktabs = T, escape = F, linesep="") %>% kableExtra::kable_styling(bootstrap_options = "striped", full_width = F, position = "left")
    )
  }
} 
```
