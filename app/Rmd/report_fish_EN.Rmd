...
Contact: [DFO.MAR-PED-Data-Request-Demande-de-donnes-DEP-MAR.MPO\@dfo-mpo.gc.ca](mailto:DFO.MAR-PED-Data-Request-Demande-de-donnes-DEP-MAR.MPO@dfo-mpo.gc.ca){.email}\
...
Contact: [Claire.Mussells\@dfo-mpo.gc.ca](mailto:Claire.Mussells@dfo-mpo.gc.ca){.email}\
...
isdbClipped <- master_intersect(isdb2015_demo, region, studyArea, mapBbox = bboxMap, Year = minYear)
marfisClipped <- master_intersect(marfis2015_demo, region, studyArea, mapBbox = bboxMap, Year = minYear)

# generate tables here
if (!is.null(isdbClipped)) {
  isdbTableList <- create_table_ISDB(isdbClipped$studyData)
  isdbAllTable <- isdbTableList$allSpecies
  isdbSARTable <- isdbTableList$sarSpecies
}

if (!is.null(marfisClipped)) {
  marfisTableList <- create_table_MARFIS(marfisClipped$studyData)
  marfisAllTable <- marfisTableList$allSpecies
  marfisSARTable <- marfisTableList$sarSpecies
}
```
...
Report_isdb <- if (is.null(isdbClipped)) {
...
Report_marfis <- if (is.null(marfisClipped)) {
...
```{r marfis-isdb-plot, fig.height=8, fig.width=11, fig.cap='Quality Tier: Medium. Security level: Protected B. Map showing the search area defined by the user (red outline) used to query aggregated information from Maritimes Fishery Information System (MARFIS) and/or Industry Survey Database (ISDB) observation records shown by species. Rule of five was not applied.'}
if (!is.null(isdbClipped)) {
  specList <- filter(ISSPECIESCODES, get("Scientific Name") %in% listed_species$`Scientific Name`)$SPECCD_ID
  plot_marfis_grid(areaMap, isdbClipped$mapData, specList, marfis = FALSE)
} 
if (!is.null(marfisClipped)) {
  specList <- filter(MARFISSPECIESCODES, COMMONNAME %in% listed_species$Common_Name_MARFIS)$SPECIES_CODE
  plot_marfis_grid(areaMap, marfisClipped$mapData, specList, marfis = TRUE)
}
```
...
outputList <- master_intersect(obis_fish_sf, region, studyArea, mapBbox = bboxMap, Year = minYear)
Samples_bbox <- outputList$mapPoints
