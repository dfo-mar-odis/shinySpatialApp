...
EBSAoutput <- master_intersect(EBSA_sf, region, studyArea, bboxMap)
...
if (!is.null(EBSAoutput$regionData)){
  plot_polygons(regionMap, regionBox, studyBox_geom, EBSAoutput$regionData, attribute)
}
```
...
if(!is.null(EBSAoutput$studyData)){print(
  plot_polygons(areaMap,bboxMap,studyBox_geom, EBSAoutput$mapData,attribute))}
```
