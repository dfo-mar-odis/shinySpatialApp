...
```
<br>
```{r crithab-area-plot, fig.height=6, fig.width=11, fig.cap=paste(write_caption_blurb(crithab_rr, "EN"), 'This map summarizes areas where the search polygon overlaps with critical habitat of Species at Risk listed as Endangered or Threatened under the <i>Species at Risk Act</i> (SARA)')}
...
```
<br>
```{r leatherback-area-plot, fig.height=6, fig.width=11, fig.cap=paste(write_caption_blurb(leatherback_rr, "EN"), 'This map summarizes areas where the search polygon overlaps with draft critical habitat of Leatherback Sea Turtle.')}
