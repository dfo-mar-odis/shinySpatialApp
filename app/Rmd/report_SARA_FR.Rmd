...
```{r sara-loaddata, results='asis'}
filesLoaded <- load_rdata(c("sardist_rr", "crithab_rr", "leatherback_rr"), "MAR", env = environment())
```
...
outputListLB <- master_intersect(leatherback_sf, mapDataList)

critHabCheck <- !is.null(outputListCrithab$studyData) | !is.null(outputListLB$studyData)

distTable <- table_dist(outputListSardist$studyData, "FR")
summaryHabTable <- add_to_hab_summary(summaryHabTable, "range", "[SAR Distribution](#sardist-section)",  distTable, "Nom Commun", "Nom Commun")
