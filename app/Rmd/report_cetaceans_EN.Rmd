...
whaleSummaryTable <- data.frame("Species" = names(whaleShape))
```
...
if (!is.null(wsdbClipped$mapData)) {
  wsdb_points <- sfcoords_as_cols(wsdbClipped$mapData)
}
...
if (!is.null(wsdbClipped$studyData)) {
  plot_rr_sf(areaMap, wsdb_points, attribute="Legend", colorMap = whaleCol, shapeMap = whaleShape)
}
```
