...
```
*
```{r mpa-check, echo=FALSE, results='asis'}
#function for overlap
...
mpaCheckOuput<-noquote(mpaCheckOuput)
...
if (!is.null(mpaClipped$regionData)){
  plot_rr_sf(regionMap, mpaClipped$regionData, mpa_rr$attribute)
}
```
...
if(!is.null(mpaClipped$studyData)){
  plot_rr_sf(areaMap, mpaClipped$mapData,  mpa_rr$attribute)
}
```
