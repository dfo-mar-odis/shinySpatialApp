...
filesLoaded <- load_rdata(c("wsdb_rr", "whitehead_rr", "narwc_rr", "obisCet_rr", 
                            "finWhale_rr", "seiWhale_rr", "humpbackWhale_rr",
                           "harbourPorpoise_rr", "nbw_rr", "bwhab_rr", 
                           "nbw_rr"), "MAR", env = environment())
...
writeLines(noquote(finWhale_rr$text$fr))
...
clipped <- master_intersect(bwhab_rr$data_sf, mapDataList)
...
if (!is.null(bwhab_rr$data_sf)){
  bwPlotList <- plot_rr_sf(regionMap, bwhab_rr$data_sf, bwhab_rr$attribute, getColorMap = TRUE)
  bwPlotList$polyMap
}  
```
...
if (!is.null(clipped$mapData)){
  plot_rr_sf(areaMap, clipped$mapData, bwhab_rr$attribute, colorMap = bwPlotList$colorMap)
}
```
